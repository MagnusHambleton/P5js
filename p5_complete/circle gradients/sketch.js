...
function draw_gradientshape(starting_color, x_pos, y_pos, d, thickness, shape) {
  red_col = red (starting_color );
  green_col = green( starting_color );
  blue_col = blue( starting_color );
  step_size = (255-min( red_col,green_col, blue_col )) / (thickness*4);

  for( let r = d; r > d-thickness*2; r--) {
    col = color(red_col, green_col, blue_col);
  ...
  function draw_shape(color, x_pos, y_pos, d, shape) {
    fill(color);
    if(shape === 'circle') {
      ellipse(x_pos, y_pos, d, d);
    } if (shape === 'arc') {
      arc(x_pos, y_pos, d, d, HALF_PI, 3*HALF_PI);
    }
  }
}
